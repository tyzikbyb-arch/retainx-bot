...
# ── Global state reset for panel buttons ─────────────────────
PANEL_BUTTONS = {
    "⌂  Main Menu", "Main Menu",
    "◈  Wallet", "Wallet",
    "▸  Video", "Video",
...
async def panel_router(msg: Message, state: FSMContext):
    """Route panel button presses regardless of current state."""
    await state.clear()
    text = msg.text.strip().replace("⌂  ", "").replace("◈  ", "").replace("▸  ", "").replace("≡  ", "").replace("≡ ", "").replace("◌  ", "").strip()
...
async def admin_all_orders(msg: Message):
    ...
    try:
        ...
        for o in orders[-10:][::-1]:
            status = "✓" if o["status"] == "delivered" else ("✕" if o["status"] == "cancelled" else "○")
            text += f"  {status}  #{o['id']}  {o['tool']}  ·  @{o['username']}\n"
        await msg.answer(text, parse_mode="HTML")
    except Exception as e:
        await msg.answer(f"Error: {e}")

@dp.message(F.text == "✉  Message User")
async def admin_msg_prompt(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    await msg.answer(
        "Send message to user:\n"
        "<code>/msg USER_ID Your text here</code>\n\n"
        "Example:\n"
...
async def admin_addcoins_prompt(msg: Message):
    if msg.from_user.id != ADMIN_ID:
        return
    await msg.answer(
        "Add coins to user:\n"
...
@dp.message(F.text == "📤  Deliver")
async def admin_deliver_prompt(msg: Message):
    if msg.from_user.id != ADMIN_ID:
        return
    await msg.answer(
...
@dp.message(F.text == "✕  Cancel Order")
async def admin_cancel_prompt(msg: Message):
    if msg.from_user.id != ADMIN_ID:
        return
    await msg.answer(
...
@dp.message(F.text == "✓  Confirm Pay")
async def admin_confirm_prompt(msg: Message):
    if msg.from_user.id != ADMIN_ID:
        return
    await msg.answer(
        "Confirm manual payment:\n"
        "<code>/addcoins USER_ID AMOUNT</code>\n\n"
        "Or use the Confirm button on the top-up request.",
        parse_mode="HTML"
    )
...
async def cancelorder_cmd(msg: Message):
    ...
    if len(parts) < 2:
        await msg.answer("Usage: /cancelorder ORDER_ID")
        return
    try:
        oid = int(parts[1])
        ...
        await bot.send_message(
            ...
            f"  <b>{order['coins']} coins</b> have been refunded.",
            parse_mode="HTML"
        )
...
async def msg_cmd(msg: Message, state: FSMContext):
    if msg.from_user.id != ADMIN_ID:
        return
    # Usage: /msg USER_ID Your message text
    parts = msg.text.strip().split(None, 2)
    if len(parts) < 3:
        await msg.answer(
            "Usage: <code>/msg USER_ID Text</code>\n"
            "Example: <code>/msg 939285095 Your order is ready!</code>",
            parse_mode="HTML"
        )
        return
    try:
...
async def main():
    ...
    # Delete webhook and drop pending updates to avoid conflicts
