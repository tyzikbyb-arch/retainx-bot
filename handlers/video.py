...
@router.message(VideoStates.collecting_start, F.photo | F.document)
...
@router.message(VideoStates.collecting_end, F.photo | F.document)
...
@router.message(VideoStates.collecting_imgs, F.photo | F.document)
...
@router.message(VideoStates.collecting_vids, F.video | F.document | F.animation)
...
@router.message(VideoStates.collecting_audio, F.audio | F.voice | F.document)
...
async def att_confirm_no_prompt(cb: CallbackQuery, state: FSMContext):
    ...
    if vid_required and not vids:
        await cb.answer("Please upload a video first.", show_alert=True)
        return

    await state.update_data(v_prompt="—")
    # Trigger confirmation directly
    await _do_confirm(cb, state)
