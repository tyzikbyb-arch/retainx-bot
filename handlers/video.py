...
class VideoStates(StatesGroup):
    ...
    # Seedance 2.0 attachment states
    sd_attach_mode = State()       # choosing attachment type
    sd_collecting_start = State()  # collecting start frame
    sd_collecting_end = State()    # collecting end frame
    sd_collecting_imgs = State()   # collecting image references
    sd_collecting_vids = State()   # collecting video references
    sd_collecting_audio = State()  # collecting audio files
    sd_entering_prompt = State()   # final prompt entry for seedance
...
# ── Tool selected ─────────────────────────────────────────────
def _sd_attach_menu(coins: int) -> str:
    return (
        "◈  <b>Seedance 2.0</b>\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
...
def _sd_attach_kb(data: dict) -> list:
    ...
    buttons.append([back_btn("vt_sd20"), menu_btn()])
    return buttons
...
async def tool_selected(cb: CallbackQuery, state: FSMContext):
    ...
    # Seedance 2.0 — show attachment menu first
    if tid == "sd20":
        data = await state.get_data()
        coins_est = usd_to_coins(0.60)  # default 720p 6sec estimate
        await state.update_data(sd_mode="free", sd_imgs=[], sd_vids=[], sd_auds=[],
                                sd_start=None, sd_end=None)
        await cb.message.edit_text(
            _sd_attach_menu(coins_est),
            reply_markup=kb(*_sd_attach_kb(await state.get_data())),
            parse_mode="HTML"
        )
...
async def prompt_received(msg: Message, state: FSMContext):
    ...
    # Check attachments for Seedance
    attach_summary = ""
    if tool == "Seedance 2.0":
        ...
        if start: attach_summary += "  ◈  Start frame\n"
        if end:   attach_summary += "  ◈  End frame\n"
        if imgs:  attach_summary += f"  ◈  {len(imgs)} image ref(s)\n"
        if vids:  attach_summary += f"  ◈  {len(vids)} video ref(s)\n"
        if auds:  attach_summary += f"  ◈  {len(auds)} audio file(s)\n"
...
async def vid_confirm(cb: CallbackQuery, state: FSMContext):
    ...
    if not tool or tool == "—" or coins == 0 or not prompt or prompt == "—":
        await cb.answer(
...
async def notify_admin(cb, oid, tool, params, coins, usd):
    ...
    if attachments:
        ...
        try:
            if start:
                if start["type"] == "photo":
                    await bot.send_photo(ADMIN_ID, start["file_id"], caption="◈  Start Frame  (@start)")
                else:
                    await bot.send_document(ADMIN_ID, start["file_id"], caption="◈  Start Frame  (@start)")
            if end:
                if end["type"] == "photo":
                    await bot.send_photo(ADMIN_ID, end["file_id"], caption="◈  End Frame  (@end)")
                else:
                    await bot.send_document(ADMIN_ID, end["file_id"], caption="◈  End Frame  (@end)")
            for i, img in enumerate(imgs, 1):
                if img["type"] == "photo":
                    await bot.send_photo(ADMIN_ID, img["file_id"], caption=f"◈  Image Ref  @img{i}")
                else:
                    await bot.send_document(ADMIN_ID, img["file_id"], caption=f"◈  Image Ref  @img{i}")
            for i, vid in enumerate(vids, 1):
                if vid["type"] == "video":
                    await bot.send_video(ADMIN_ID, vid["file_id"], caption=f"◈  Video Ref  @vid{i}")
                else:
                    await bot.send_document(ADMIN_ID, vid["file_id"], caption=f"◈  Video Ref  @vid{i}")
            for i, aud in enumerate(auds, 1):
                if aud["type"] == "audio":
                    await bot.send_audio(ADMIN_ID, aud["file_id"], caption=f"◈  Audio  @aud{i}")
                elif aud["type"] == "voice":
                    await bot.send_voice(ADMIN_ID, aud["file_id"], caption=f"◈  Audio  @aud{i}")
                else:
                    await bot.send_document(ADMIN_ID, aud["file_id"], caption=f"◈  Audio  @aud{i}")
        except Exception as e:
            await bot.send_message(ADMIN_ID, f"⚠️  Error sending attachments: {e}")
...
@router.message(VideoStates.sd_collecting_start, F.photo | F.document)
...
@router.message(VideoStates.sd_collecting_end, F.photo | F.document)
...
@router.message(VideoStates.sd_collecting_imgs, F.photo | F.document)
...
@router.message(VideoStates.sd_collecting_vids, F.video | F.document)
...
@router.message(VideoStates.sd_collecting_audio, F.audio | F.voice | F.document)
