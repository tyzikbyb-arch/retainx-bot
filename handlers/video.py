...
VIDEO_SUBCATS = {
    ...
    "Avatar":    ["hga4","hgtr","eldb","lips","omni","aur1","grok"],
}
SUBCAT_LABELS = {
...
def _build_attach_buttons(tid: str, data: dict) -> list:
    ...
    if no_prompt:
        if vid_required and not vids:
            proceed_label = "⚠️  Upload video first"
        else:
            proceed_label = "◈  Confirm Order" if has_any else "◈  Confirm Order"
...
async def show_omni_dur(cb, state):
    buttons = [
        [InlineKeyboardButton(text=f"15 sec — {OMNIHUMAN_PRICES[15]}◈", callback_data="omni_dur_15")],
        [InlineKeyboardButton(text=f"30 sec — {OMNIHUMAN_PRICES[30]}◈", callback_data="omni_dur_30")],
        [back_btn("vsub_Avatar"), menu_btn()],
    ]
    await cb.message.edit_text(
        "◈  <b>OmniHuman 1.5</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
...
async def omni_dur(cb: CallbackQuery, state: FSMContext):
    sec = int(cb.data[9:])
    coins = OMNIHUMAN_PRICES[sec]
    await state.update_data(
        v_tool="OmniHuman 1.5", v_tid="omni",
        v_dur=sec, v_coins=coins, v_usd=round(coins*0.05, 2),
        att_mode="free", att_start=None, att_end=None,
        att_imgs=[], att_vids=[], att_auds=[],
    )
    await _show_attach_menu(cb, state)
...
async def aur1_start(cb: CallbackQuery, state: FSMContext):
    ...
    buttons = [[InlineKeyboardButton(text=ar, callback_data=f"aur1_ar_{ar}")] for ar in HEYGEN_AVATAR_ASPECT_RATIOS]
    buttons.append([back_btn("vsub_Avatar"), menu_btn()])
    await cb.message.edit_text(
        "◈  <b>Aurora Avatar</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
        "  Select aspect ratio:",
...
@router.callback_query(F.data.startswith("aur1_ar_"))
async def aur1_ar(cb: CallbackQuery, state: FSMContext):
    ar = cb.data[8:]
    await state.update_data(v_ar=ar)
    buttons = [[InlineKeyboardButton(text=r, callback_data=f"aur1_res_{r}")] for r in HEYGEN_AVATAR_RESOLUTIONS]
    buttons.append([back_btn("vt_aur1"), menu_btn()])
    await cb.message.edit_text(
        f"◈  <b>Aurora Avatar</b>  —  {ar}\n━━━━━━━━━━━━━━━━━━━━\n\n"
        "  Select resolution:",
        reply_markup=kb(*buttons), parse_mode="HTML"
    )

@router.callback_query(F.data.startswith("aur1_res_"))
async def aur1_res(cb: CallbackQuery, state: FSMContext):
    res = cb.data[9:]
    await state.update_data(v_res=res)
    data = await state.get_data()
    ar = data.get("v_ar", "—")
    ...
    rows.append([back_btn(f"aur1_ar_{ar}"), menu_btn()])
    await cb.message.edit_text(
        f"◈  <b>Aurora Avatar</b>  —  {ar}  {res}\n━━━━━━━━━━━━━━━━━━━━\n\n"
        "  Select duration:",
        reply_markup=kb(*rows), parse_mode="HTML"
    )
