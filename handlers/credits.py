...
async def receive_custom_amount(msg: Message, state: FSMContext):
    try:
        amount = float(msg.text.replace(",", ".").replace("$", "").strip())
        if amount < MIN_TOPUP_USD:
            await msg.answer(f"Minimum deposit is ${MIN_TOPUP_USD}. Please enter a valid amount.")
            return
        await state.clear()
        await show_payment_options_msg(msg, state, amount)
    except ValueError:
        await msg.answer("Please enter a valid number (e.g. 5 or 7.50).")
...
async def pay_usdt(cb: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(TopupStates.entering_amount)
    await state.update_data(awaiting_tx=True)
