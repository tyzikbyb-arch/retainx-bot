...
CREDIT_PACKS = {
    20: 100, 100: 500, 200: 1000, 500: 2500, 1000: 4500, 2000: 8000,
}
...
async def create_checkout(body: CheckoutRequest, user: User = Depends(get_current_user)):
    ...
    session = stripe.checkout.Session.create(
        ...
        line_items=[{"price_data": {"currency": "usd", "unit_amount": CREDIT_PACKS[body.credits], "product_data": {"name": f"RetainX Studio — {body.credits} Credits", "description": f"{body.credits} credits · 1 credit = $0.05"}}, "quantity": 1}],
        mode="payment",
