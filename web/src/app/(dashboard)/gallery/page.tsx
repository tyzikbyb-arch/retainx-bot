  ...
  return (
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-black mb-1">Community Gallery</h1>
          <p style={{ color: "var(--text-secondary)" }}>Discover what others are creating. All prompts are public.</p>
        </div>
        <div className="flex gap-2">
          {["All","Video","Image","Audio"].map((t) => (
            <button key={t} className="px-4 py-2 rounded-xl text-sm font-medium" style={t==="All" ? {background:"var(--accent)",color:"#fff",border:"none",cursor:"pointer"} : {background:"var(--bg-card)",color:"var(--text-secondary)",border:"1px solid var(--border)",cursor:"pointer"}}>{t}</button>
          ))}
        </div>
      </div>
          ...
          <div key={i} className="break-inside-avoid rounded-2xl overflow-hidden group cursor-pointer" style={{ border: "1px solid var(--border)" }}>
            <div className="w-full relative" style={{ height: i%3===0?200:i%3===1?160:240, background: item.gradient }}>
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3" style={{ background: "rgba(0,0,0,0.6)" }}>
                <button className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", cursor: "pointer" }}><Heart size={16} color="#fff" /></button>
                <button className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", cursor: "pointer" }}><Download size={16} color="#fff" /></button>
              </div>
            </div>
            <div className="p-4" style={{ background: "var(--bg-card)" }}>
              <div className="flex items-center gap-2 mb-2">
                <div className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: (typeColor[item.type]||"#7c3aed")+"22", color: typeColor[item.type]||"#7c3aed" }}>
                  {item.type==="video"?<Video size={11}/>:item.type==="image"?<ImageIcon size={11}/>:<Mic size={11}/>}
